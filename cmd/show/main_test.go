...
// readSSEChunks connects to url with optional lastID and returns concatenated
// data from 'chunk' events until ctx cancel or 'done' event is seen.
func readSSEChunks(t *testing.T, client *http.Client, url string, lastID string, maxBytes int) ([]byte, bool) {
    t.Helper()
    req, err := http.NewRequest("GET", url, nil)
    if err != nil { t.Fatal(err) }
    if lastID != "" { req.Header.Set("Last-Event-ID", lastID) }
    ...
    for {
        ...
        if line == "" { // dispatch event
            if curEvent == "chunk" && curData != "" {
                ...
                buf = append(buf, dec...)
                if maxBytes > 0 && len(buf) >= maxBytes { return buf, haveDone }
            }
            if curType == "done" { haveDone = true; return buf, haveDone }
...
// base64NewDecoder returns a Decoder that decodes the provided base64 string.
...
func TestBacklogReplay(t *testing.T) {
    ...
    if done { t.Fatalf("unexpected done event") }
}
